...
- **Shape real do "Parar Loop" (`.utility.loop.BreakLoop`) em `build-flow`.** O `stepSkeleton` do catalogo nao traz `image`; um BreakLoop montado a partir dele (sem `image`, com `nextSteps: []` e campos genericos) foi aceito no save mas teve de ser recriado no canvas. Documentado o shape que a UI gera (`image: "break-loop"`, `label: "Parar Loop"`, terminal, sem `nextSteps`), como o step anterior aponta para ele, e nova linha na tabela de tipos fixos.
- **BreakLoop validado em execucao e `DELAY` documentado em `build-flow`.** O BreakLoop foi exercitado num retry real (`TIMES` 3): encerra o loop inteiro e o fluxo segue para o `nextSteps` do `LoopCanvas`, inclusive com saida antecipada (sucesso na 1a tentativa terminou o loop ja na 1a iteracao). Documentado o shape do `DELAY` (`actionId DELAY`, `coreRouteType DELAY_UTILITY`, `inputData.timeinMilliseconds` com interpolacao), tambem validado em execucao, e o padrao NodeJS (calcula `delayMs`) -> Delay -> `.StopLoop` para backoff.
...
- **Padrao "RunChildFlow — orquestrador + subfluxos" em `apipass-patterns`**, com o shape de `SwitchUtility` + `RunChildFlowUtility` (`async: true`) dentro de um `LoopCanvas`, e principalmente o **criterio de decisao entre subfluxo DEDICADO por item (1 flowId por item, isolamento total de fila/historico) vs. subfluxo GENERICO reaproveitado (1 flowId parametrizado)** — tabela comparativa + regra pratica de quando perguntar ao usuario em vez de assumir. Motivado por uma correcao ao vivo do cliente no projeto "Vale": a arquitetura generica foi montada por padrao e teve que ser refeita como dedicada porque o requisito real era isolamento de falha por arquivo, algo que nao estava explicito no pedido inicial.
### Corrigido
- **`RunChildFlowUtility` pode parar de disparar um `flowId` especifico (sync trava, async nao deixa rastro) por estado corrompido na plataforma**, mesmo com config identica a um clone que funciona e o mesmo flow rodando OK via `run_test_flow` direto. Nova linha na tabela de `apipass-gotchas`, com referencia cruzada em `apipass-patterns`. Observado apos um historico pesado de `stop_execution`/republicacoes no mesmo flow; resolvido recriando o flow com `flowId` novo.
- **Idempotencia em acao de catalogo (`idempotencyStrategy`/`idempotencyTtl`) nao ativa sem o array `resources: ["IDEMPOTENCY"]`** no nivel superior do step — o save aceita sem erro, mas a dedup simplesmente nao acontece. Nova linha em `apipass-gotchas`.
- **`FTP_UPLOAD_FILE` trava silenciosamente (sem erro, sem respeitar timeout) em upload de arquivo grande pra um servidor SFTP especifico** — causa raiz era a cifra default da credencial, nao tamanho de arquivo nem timeout. Fix: habilitar `enableAESGCMSupport` (AES-256 GCM) na credencial FTP antes de escalar pra plataforma. Nova linha em `apipass-gotchas`.
