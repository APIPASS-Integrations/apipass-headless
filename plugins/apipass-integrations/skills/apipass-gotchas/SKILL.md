...
| Variavel nova (ex. `{{$.stage.MINHA_VAR_NOVA}}`) nao aparece em `get_environment`, e `update_environment_variables` a ignora silenciosamente (so relata as chaves que reconhece) | Um environment so herda as "definitions" de variavel **ja cadastradas na conta** — nao existe tool MCP para criar uma definition nova | Peca ao usuario (ou ao admin da conta) para cadastrar a variavel na tela de configuracoes da conta, fora do MCP, pela interface web. So depois disso ela passa a existir em todo environment e pode ser preenchida via `update_environment_variables` |
| `update_environment_variables` retorna HTTP 400 `flowEngineId must be a UUID` mesmo o environment tendo um `flowEngineId` valido (confirmavel em `get_environment`) | Bug conhecido da tool, observado em environments recem-criados | Nao insista em retry — peca ao usuario para preencher o(s) valor(es) direto na interface web da APIPASS |
